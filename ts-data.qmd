...
The conventional 95% confidence bands are thus $\pm 1.96 / \sqrt{T}$, often displayed as blue dashed lines. Sample ACF values outside these bands suggest statistically significant autocorrelation at that lag.

```{r}
#| label: fig-acf-soi-annotated
#| fig-cap: "Sample ACF with confidence bands"

soi_acf <- acf1(soi, main = "SOI: Sample ACF")
```
...
- **Slow ACF decay** corresponds to **low-frequency dominance** in the spectrum (trend or long-memory behavior).
...
- **Gradual ACF decay** corresponds to **peaked spectrum** (AR-type behavior).
...
Box.test(soi, lag = 20, type = "Ljung-Box")
```
...
These are not minor technicalities. In fields where time series data are common—economics, climate science, epidemiology, finance—failure to account for autocorrelation has led to overconfident conclusions and spurious findings.
...
**Block bootstrap**: Resampling methods that preserve the dependence structure can provide valid inference without parametric assumptions.
...
2. **Two equivalent descriptions**: The autocorrelation function (time domain) and spectral density (frequency domain) contain the same information, expressed differently. They are Fourier transform pairs.
