...
1. **Explain** why clustering is useful for exploring high-dimensional data where direct visualization fails
2. **Construct** grouping variables from continuous measurements and interpret the resulting groups
3. **Apply** z-score standardization to prepare variables for distance-based clustering
4. **Execute** K-means clustering using `stats::kmeans()`, including the use of multiple random starts (`nstart`)
5. **Interpret** within-cluster sum of squares (WCSS) and use the elbow method to guide the choice of $K$
6. **Compare** clusterings using cross-tabulation and Jaccard similarity
7. **Distinguish** between centroid-based (K-means) and density-based (DBSCAN) clustering approaches
8. **Translate** cluster assignments into substantive interpretations using domain knowledge
:::
...
group_norms_lst$grp_norms |> 
  ggplot2::ggplot(mapping = ggplot2::aes(y = norm)) + 
  ggplot2::geom_boxplot() + 
  ggplot2::facet_grid(cols = ggplot2::vars(grp_lbl)) + 
  ggplot2::labs(
...
cluster_norms_lst$grp_norms |> 
  ggplot2::ggplot(mapping = ggplot2::aes(y = norm)) + 
  ggplot2::geom_boxplot() + 
  ggplot2::facet_grid(cols = ggplot2::vars(cluster)) + 
  ggplot2::labs(
...
xtabs_cl_group_nstart  <- stats::xtabs(
  data    = college_z_scores |> 
  dplyr::mutate(
    cl_group  = m_kmeans_ctrs$cluster |> forcats::as_factor(), 
    cl_nstart = m_kmeans_nstart$cluster |> forcats::as_factor()
  ), 
...
ggplot2::ggplot(wcss_by_k, ggplot2::aes(x = k, y = wcss)) +
  ...
  ggplot2::labs(x = "Number of clusters (K)", y = "Within-cluster SS")
```
