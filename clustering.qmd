...
Team up with a classmate and make your own copy of the `ISLR2::College` data. Spend 15 minutes exploring the data and preparing to report out to the class. Consider the following questions to guide your exploration:

::: {.callout-tip}
## Guiding questions

1. **Composition:** How many private vs. public schools are in the data?
2. **Scale:** What is the range of `Expend` (expenditure per student)? How does this compare to the range of `Grad.Rate`?
3. **Association:** Which numeric variables appear to be correlated? Use `GGally::ggpairs()` on a subset of variables.
4. **Expectations:** Based on your domain knowledge, what groupings would you expect to find among colleges? Elite vs. non-elite? Large vs. small? Research-focused vs. teaching-focused?
5. **Questions:** What questions about colleges might clustering help answer?
:::

Record your observations and conjectures. Which of these could be addressed by the available variables?
...
3.  **Wine quality data (optional):** The `eda4mldata::wine_quality` dataset contains chemical measurements of wines. Apply K-means clustering to the numeric variables and interpret the resulting clusters. Do they correspond to quality ratings or wine color?
