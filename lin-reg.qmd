...
# Load Galton family heights from eda4mlr
data(galton_families, package = "eda4mlr")

# Load OECD Better Life Index
data(oecd_bli, package = "eda4mlr")
```
...
9. Apply the Pythagorean decomposition to relate total, explained, and residual variance.
:::
...
We can check for multicollinearity by examining the correlation between feature vectors. In the Galton data, the correlation between mother's and father's heights is:
...
This positive correlation reflects assortative mating—taller people tend to marry taller people—but it is not high enough to cause serious multicollinearity.
...
Using the `galton_families` data from `eda4mlr`:
