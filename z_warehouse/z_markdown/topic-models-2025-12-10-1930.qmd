...
(a) Roll the topic dice according to $\boldsymbol{\gamma}_d$ to select a topic.
...
These interpretations require domain knowledge. The algorithm provides word distributions; the analyst provides labels. Note that the 1988 publication date explains the prominence of Cold War vocabulary in Topic 3.
...
ap_gamma_k4 |>
  ...
  dplyr::rename(Document = document) |>
  knitr::kable(
    digits = 2,
    col.names = c("Document", "Topic 1", "Topic 2", "Topic 3", "Topic 4")
  )
```
...
LDA assumes topic mixtures are drawn from a *Dirichlet distribution*—a probability distribution whose outcomes are themselves probability vectors. Just as a normal distribution generates real numbers, the Dirichlet distribution generates vectors that sum to 1.
...
1. One controls topic mixtures per document (typically called $\alpha$). Small values mean documents focus on few topics.

2. One controls word mixtures per topic (sometimes called $\eta$ or $\delta$). Small values mean topics focus on specific words rather than spreading probability across the vocabulary.

These are hyperparameters that can be tuned, though default values often work well. For a fuller mathematical treatment of the Dirichlet distribution, including its density function and conjugacy to the multinomial distribution, see Appendix 11.A.
...
1. For each topic $k \in \{1, \ldots, K\}$: Draw word distribution $\boldsymbol{\beta}_k \sim \text{Dir}(\boldsymbol{\eta})$.

2. For each document $d \in \mathcal{D}$:
   (a) Draw topic mixture $\boldsymbol{\gamma}_d \sim \text{Dir}(\boldsymbol{\alpha})$.
...
For further details on inference algorithms, see @Steyvers_Griffiths_2007 and @BNJ2003LDA.
...
2. **Document Mixtures.** For a document with $\boldsymbol{\gamma} = (0.4, 0.3, 0.2, 0.1)$: (a) What does this mixture tell you about the document's content? (b) How would you explain this to a non-technical colleague? (c) Would you classify this document into one topic or multiple? What are the tradeoffs?
...
- @BNJ2003LDA. Latent Dirichlet Allocation. *Journal of Machine Learning Research*. The original LDA paper.
...
- Silge and Robinson. [Text Mining with R](https://www.tidytextmining.com/), Chapter 6. Tidy approach to topic modeling in R.
...
## Appendix 11.A: The Dirichlet Distribution {.appendix}
...
$$
P(\mathbf{S} = \mathbf{s} \mid \mathbf{p}) = \binom{n}{\mathbf{s}} \prod_{k=1}^{K} p_k^{s_k}
$$ {#eq-multinomial}
...
$$
P(\mathbf{p}) = \frac{\Gamma(\alpha_1 + \cdots + \alpha_K)}{\Gamma(\alpha_1) \times \cdots \times \Gamma(\alpha_K)} \prod_{k=1}^{K} p_k^{\alpha_k - 1}
...
$$
\mathbf{p} \mid \mathbf{s} \sim \text{Dir}(\boldsymbol{\alpha} + \mathbf{s})
...
1. **Document-topic mixtures**: Each document's topic proportions $\boldsymbol{\gamma}_d$ are drawn from $\text{Dir}(\boldsymbol{\alpha})$. A small $\alpha$ encourages documents to focus on few topics.

2. **Topic-word distributions**: Each topic's word probabilities $\boldsymbol{\beta}_k$ are drawn from $\text{Dir}(\boldsymbol{\eta})$. A small $\eta$ encourages topics to assign high probability to few words.
...
- @wiki_Dirichlet_dstn for an accessible introduction.
- @BNJ2003LDA for the original application to topic modeling.
- @Steyvers_Griffiths_2007 for details on the Gibbs sampling algorithm.
