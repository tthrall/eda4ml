...
## Computation in R {#sec-graph-theory-computation}
...
The examples in this chapter use the `igraph` package for graph analysis and `igraphdata` for built-in datasets. The LearningGraph example uses the `eda4mldata` package.
...
# Install eda4mldata from GitHub
...
Graphs can be constructed from edge lists, adjacency matrices, or built-in datasets.

```{r}
#| label: computation-construction
#| eval: false
...
igraph::is_connected(karate)
```

### Centrality Measures

The four centrality measures discussed in this chapter each answer a different question about node importance.

```{r}
#| label: computation-centrality
#| eval: false
...
igraph::page_rank(karate)$vector
```

### Community Detection

```{r}
#| label: computation-community
#| eval: false
...
length(comm_louvain)                  # Number of communities detected
```

### Paths and Distances

```{r}
#| label: computation-paths
#| eval: false
...
igraph::topo_sort(g_dag)
```

### Bipartite Graphs and Projections

```{r}
#| label: computation-bipartite
#| eval: false
...
igraph::E(user_proj)$weight
```

### Building a Feature Matrix

For machine learning tasks, we often need to convert graph structure into a feature matrix with one row per node.

```{r}
#| label: computation-features
...
This feature matrix can be joined with node labels (e.g., faction membership) for supervised learning, or used directly for clustering or anomaly detection.
...
[A User's Guide to Network Analysis in R](https://link.springer.com/book/10.1007/978-3-319-23883-8) by Douglas A. Luke — practical introduction to `igraph` and network analysis.
 
[Network Science](http://networksciencebook.com/) by Albert-László Barabási — comprehensive textbook, freely available online.

[igraph R manual](https://r.igraph.org/) — official documentation for the `igraph` package.

[Stanford Network Analysis Project (SNAP)](https://snap.stanford.edu/) — large-scale network datasets and tools.

[The Anatomy of a Large-Scale Hypertextual Web Search Engine](http://infolab.stanford.edu/~backrub/google.html) by Brin and Page — the original PageRank paper.

[Community detection in graphs](https://arxiv.org/abs/0906.0612) by Fortunato — comprehensive survey of community detection methods.
