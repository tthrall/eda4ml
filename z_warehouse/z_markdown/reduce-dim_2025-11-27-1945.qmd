...
- **Partial least squares (PLS)**: PLS finds linear combinations of predictors that have high covariance with the response.  Unlike PCA (which ignores $Y$) or ordinary regression (which may overfit when $d$ is large), PLS balances explanatory power for $X$ and predictive power for $Y$. [TODO: connection to canonical correlation?]

- **Sufficient dimension reduction**: Methods such as sliced inverse regression (SIR) seek the lowest-dimensional subspace of the predictors that captures all information relevant to predicting $Y$.  These methods generalize Fisher's discriminant analysis to continuous responses and more flexible model structures. [TODO: connection to older methods like subset regression?]
...
- **Kernel PCA**: By applying PCA in a high-dimensional feature space induced by a kernel function, kernel PCA can capture nonlinear relationships.  The "kernel trick" avoids explicitly computing coordinates in the high-dimensional space. [TODO: TT homework]
...
- **Autoencoders**: A neural network architecture that learns to compress data to a low-dimensional "bottleneck" layer and then reconstruct it.  Autoencoders can learn highly nonlinear transformations but require substantial data and computation. [TODO: TT homework]

### What Is Preserved?

Different methods optimize different objectives.  That knowledge can help one to decide which methods to use.  Here's a table that may serve as a starting point. [TODO: What other methods should be mentioned?  Of the methods mentioned in section 7.5, what is the principle for listing a method in this table?]
...
method_objectives_tbl <- tibble::tibble(
  tag = 
    c("pca", "lda", "fct_an", "mds", "tsne", "log_reg", "pls"), 
  nm_short = 
    c("PCA", "LDA", "Factor Analysis", "MDS", "t-SNE", "Logistic Regression", "PLS"), 
  presrv = 
    c("Variance", "Class separation", "Common variance", "Pairwise distances", "Local neighborhoods", "Class boundaries", "Covariance with Y"), 
  is_supvd = 
    c(FALSE, TRUE, FALSE, FALSE, FALSE, TRUE, TRUE), 
  is_lin = 
    c(TRUE, TRUE, TRUE, NA, FALSE, TRUE, TRUE)
) |> 
  dplyr::mutate(
    chr_supvd = is_supvd |> 
...
#| tbl-cap: "Examples of methods and their objectives (preserved property)."
...
method_objectives_tbl |> 
  ...
  dplyr::rename(
    ...
    `Linear?`     = chr_lin ) |> 
  knitr::kable(
    caption = "Examples of methods and objectives (preserved property)"
  )
...
- **Random projections** offer a computationally cheap alternative: the Johnson-Lindenstrauss lemma guarantees that projecting onto a random low-dimensional subspace approximately preserves pairwise distances. [TODO: citation?]
...
2. **Supervision**: If class labels or a response variable are available, supervised methods generally outperform unsupervised ones for prediction tasks. [TODO: example of prediction by an unsupervised method?]
