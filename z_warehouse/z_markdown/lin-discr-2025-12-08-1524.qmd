...
We have examined PCA and LDA in depth because they are foundational, computationally tractable, and geometrically interpretable.  Many other dimension reduction methods exist, each with different objectives.

### Supervised versus Unsupervised

**Unsupervised methods** find structure in features alone:

- **PCA**: Maximises variance
- **Factor analysis**: Separates common from unique variance
- **Independent Component Analysis (ICA)**: Seeks statistically independent components
- **Clustering**: Partitions observations into groups

**Supervised methods** use response labels:

- **LDA**: Maximises between-class separation
- **Logistic regression**: Models class probabilities
- **Partial Least Squares (PLS)**: Maximises covariance with response
- **Sufficient dimension reduction**: Finds subspaces containing all predictive information

### Linear versus Nonlinear

PCA and LDA are linear: the reduced representation is a linear combination of original features.  When data lie on curved manifolds, nonlinear methods may be needed:

- **Kernel PCA**: Applies PCA in a feature space defined by a kernel function
- **t-SNE**: Preserves local neighborhood structure; excellent for visualization
- **UMAP**: Similar to t-SNE but often faster and may preserve more global structure
- **Autoencoders**: Neural networks that learn nonlinear compressions
...
1. Begin with PCA for exploratory visualization
2. If class labels are available and classification is the goal, try LDA
3. If linear methods reveal limited structure, consider t-SNE or UMAP for visualization
4. When $d > n$, regularisation becomes essential; consider penalised methods
