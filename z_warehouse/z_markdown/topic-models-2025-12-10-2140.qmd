...
| $\alpha_\bullet$ | Dirichlet parameter for document-topic mixtures |
| $\eta_\bullet$ | Dirichlet parameter for topic-word distributions |
...
| Cluster centroids | Word distributions ($\beta_{k,\bullet}$) per topic |
| Cluster assignments | Topic mixtures ($\gamma_{d,\bullet}$) per document |
...
1. **Document-topic mixtures**: Each document's topic proportions $\gamma_{d,\bullet}$ are drawn from $\text{Dir}(\alpha_\bullet)$. A small $\alpha$ encourages documents to focus on few topics.

2. **Topic-word distributions**: Each topic's word probabilities $\beta_{k,\bullet}$ are drawn from $\text{Dir}(\eta_\bullet)$. A small $\eta$ encourages topics to assign high probability to few words.
...
## Appendix 11.B: Mathematical Framework {.appendix}

For readers seeking a more formal treatment, this appendix presents the mathematical notation and generative model underlying LDA in full detail.
...
### References for Appendix 11.B

- @BNJ_2003_LDA for the original LDA paper with full mathematical details.
- @Steyvers_Griffiths_2007 for an accessible treatment of the Gibbs sampling algorithm.
