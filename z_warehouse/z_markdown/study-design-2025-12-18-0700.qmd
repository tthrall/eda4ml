...
# TODO: Capitalize surnames in political polling tables
library(eda4mldata)
...
The Silberzahn study does not counsel despair. It counsels humility and transparency. The practical implications for data analysts are threefold:

1. **Transparency.** Document analytical choices and, where feasible, the alternatives considered. Readers cannot assess robustness if they cannot see the decision points. Record not just what you did but what alternatives you considered—future readers need to understand the path you took through the garden of forking paths.

2. **Sensitivity analysis.** When key conclusions depend on contestable choices, examine how results change under alternative specifications. Stability across reasonable alternatives strengthens confidence; instability signals that conclusions are provisional.

3. **Communication.** Report uncertainty honestly. A finding that holds under one defensible specification but not others warrants different language than one that proves robust across multiple approaches. For confirmatory analyses, pre-registering the analytical plan before seeing results disciplines the process and distinguishes exploratory from confirmatory findings.
