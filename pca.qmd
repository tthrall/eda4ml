...
1. **Explain** the motivation for dimension reduction and the curse of dimensionality
2. **Interpret** principal components geometrically as directions of maximum variance
3. **Derive** principal components as eigenvectors of the covariance matrix
4. **Determine** the need for centering alone versus centering _and_ scaling features
5. **Construct** and interpret scree plots and cumulative variance plots
6. **Create** and interpret biplots showing both observations and variable loadings
7. **Connect** PCA to singular value decomposition (SVD)
8. **Apply** PCA to real datasets and interpret the results substantively
:::
...
@fig-fs-pca illustrates PCA for (father, son) centred heights from the Galton data introduced in @sec-la-intro.
