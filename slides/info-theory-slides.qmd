...
ggplot() +
  geom_circle(data = circles, aes(x0 = x0, y0 = y0, r = r, fill = label), 
              alpha = 0.5, color = "white", linewidth = 1.5) +
  annotate("label", x = -1.1, y = 0, label = "H(X|Y)", size = 5, 
           fill = "#4A90D9", color = "white", fontface = "bold", label.size = 0) +
  annotate("label", x = 1.1, y = 0, label = "H(Y|X)", size = 5, 
           fill = "#E07050", color = "white", fontface = "bold", label.size = 0) +
  annotate("label", x = 0, y = 0, label = "MI", size = 6, 
           fill = "#8B6090", color = "white", fontface = "bold", label.size = 0) +
  coord_fixed() +
  theme_void() +
  theme(legend.position = "none",
        panel.background = element_rect(fill = "gray20", color = NA)) +
  scale_fill_manual(values = c("H(X)" = "#4A90D9", "H(Y)" = "#E07050"))
```
...
The mathematical details require additional concepts (degenerate distributions) beyond our scope here.
...
3. **KL Divergence**: Verify that $KL(P \| Q) \neq KL(Q \| P)$ using the Box 5 / Box 6 example. Calculate both directions and interpret the asymmetry.
