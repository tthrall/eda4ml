...
This session highlights some basic ideas and methods that underpin a rapidly advancing field. We follow the online book by Silge and Robinson cited below and use their `R` package `tidytext`. The authors emphasize the "tidy" formatting of data (i.e., as key-value pairs) along with a set of `R` packages sharing this approach, collectively called the `tidyverse`.

When exploring a new text corpus, a systematic approach helps reveal structure and guide subsequent analysis. Start by tokenizing the text and examining word frequency distributions—what words dominate, and do they make sense given the source? Remove stop words and re-examine. Compute tf-idf to identify terms that distinguish documents within the corpus. For larger collections, consider topic models to discover latent themes. Throughout, visualize and iterate: unexpected patterns often point to data quality issues (encoding errors, boilerplate text) or genuine insights worth pursuing. The methods in this chapter provide a toolkit for this exploratory process.
...
Another common preprocessing step is *stemming* or *lemmatization*—reducing words to a common root form. Stemming applies simple rules to strip suffixes: "running", "runs", and "ran" all become "run". Lemmatization uses vocabulary knowledge to find true root forms: "better" becomes "good". These techniques can improve analysis by grouping related words, though they may obscure meaningful distinctions. The `tidytext` workflow doesn't include built-in stemming, but packages such as `SnowballC` and `textstem` provide these capabilities. We won't use stemming in this chapter, but you should know the option exists.
...
hgwells_lines <- eda4mldata::hgwells
...
# Associated Press data in tidy format (from eda4mldata)
# Original source: topicmodels::AssociatedPress
...
# LDA model with k = 2 topics, fitted using:
#   topicmodels::LDA(AssociatedPress, k = 2, control = list(seed = 1234))
# Pre-computed results provided by eda4mldata::ap_topics
```

We now examine (topic, term) probabilities (called $\beta$ in the LDA literature).
...
## Beyond Word Counts: Modern Approaches

The methods presented in this chapter treat words as discrete tokens and documents as sparse vectors of word counts. This *bag-of-words* paradigm has proven remarkably useful, but it has limitations. The phrase "not good" scores the same as "good" in a word-count analysis. Synonyms like "excellent" and "superb" appear as unrelated as "excellent" and "potato." And two documents using entirely different vocabulary to discuss the same topic will show no similarity at all.

Extensions such as *n-grams*—sequences of $n$ consecutive words—can capture phrases like "New York" or "machine learning" that single-token analysis misses. But more fundamental advances have come from learning *dense, continuous* representations that capture semantic relationships. Rather than representing a word as a sparse vector with a single 1 among thousands of zeros, these methods represent words (and sentences, and documents) as dense vectors of perhaps 100–300 real numbers. This shift from sparse to dense representation enables capabilities that word-count methods cannot achieve.

### Sentiment Analysis

Sentiment analysis assigns emotional valence to text—determining whether a passage expresses positive, negative, or neutral sentiment, and sometimes identifying specific emotions such as anger, joy, or fear. The simplest approaches use *lexicons*: curated lists of words with associated sentiment scores. The AFINN lexicon assigns numeric scores (e.g., "excellent" = +3, "terrible" = −3); the Bing lexicon classifies words as simply positive or negative; the NRC lexicon associates words with emotions like trust, anticipation, or sadness.

Lexicon-based sentiment analysis is straightforward to implement—the `tidytext` package provides access to these lexicons via `get_sentiments()`—but inherits the limitations of word-count methods. More sophisticated approaches use machine learning to account for context, negation, and sarcasm. Applications include analyzing product reviews, monitoring social media, and understanding customer feedback.

For details on sentiment lexicons and their use in `R`, see Silge and Robinson's [Text Mining with R: Sentiment Analysis](https://www.tidytextmining.com/sentiment).

### Word Embeddings

Word embeddings represent words as dense vectors learned from large text corpora. The key insight is that words appearing in similar contexts should have similar representations. A neural network trained on millions of sentences learns vectors such that semantically related words are geometrically close: "king" and "queen" are nearby, as are "Paris" and "France."

More remarkably, semantic *relationships* become geometric relationships. The vector from "man" to "woman" is approximately parallel to the vector from "king" to "queen," enabling analogical reasoning through vector arithmetic. This property emerges from the training process without explicit supervision.

From the perspective of this book, word embeddings are dimension reduction applied to vocabulary. A one-hot encoding of 50,000 words produces 50,000-dimensional sparse vectors; word embeddings compress this to perhaps 300 dimensions while preserving (and revealing) semantic structure (see @sec-reduce-dim). Three widely-used methods are:

- [Word2Vec](https://arxiv.org/abs/1301.3781) (Mikolov et al., 2013): Learns embeddings by predicting words from their context (or vice versa)
- [GloVe](https://nlp.stanford.edu/projects/glove/) (Pennington et al., 2014): Learns embeddings from global word co-occurrence statistics
- [FastText](https://fasttext.cc/) (Bojanowski et al., 2017): Extends Word2Vec by incorporating subword information, enabling embeddings for words not seen during training

### Transformers and Large Language Models

Word embeddings assign each word a single vector regardless of context—"bank" has the same representation whether discussing rivers or finance. *Transformer* architectures, introduced in 2017, produce context-sensitive representations. The same word receives different vectors depending on surrounding words, capturing polysemy and contextual nuance.

Transformers are the foundation of modern large language models (LLMs) such as GPT and BERT. These models, trained on vast text corpora, power applications including text generation, question answering, summarization, and translation. The field is evolving rapidly; capabilities that seemed futuristic a few years ago are now routine.

### Implications for Machine Learning

These modern representations serve as features for downstream ML tasks. Text classification (spam detection, topic categorization, sentiment prediction), document clustering, and semantic search all benefit from dense representations that capture meaning rather than just word occurrence.

Text can also be represented as networks, with words as nodes and co-occurrence or syntactic relationships as edges, enabling the graph-based methods discussed in @sec-graph-theory.

The exploratory techniques from this chapter—examining vocabulary, term frequencies, document-term matrices, and topic structure—remain valuable even when using modern methods. Understanding what words appear, how often, and in what combinations provides insight that informs modeling decisions. As with other data types, exploration precedes modeling.
