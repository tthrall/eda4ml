...
1. **Use** R's distribution functions (`d`/`p`/`q`/`r` prefix convention) to compute densities, probabilities, quantiles, and random samples
2. **Design** a simulation study with appropriate parameters (sample size $n$, replications $R$) and storage structures
3. **Verify** theoretical results through simulation, such as the variance ratio of sample mean versus median
4. **Apply** Monte Carlo methods to estimate quantities that are difficult to compute analytically
5. **Explain** when importance sampling is needed and implement it for rare event estimation
6. **Construct** bootstrap confidence intervals for arbitrary statistics
