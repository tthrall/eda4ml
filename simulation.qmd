...
With a teammate, verify the variance ratio result using your own simulation.

::: {.callout-tip}
## Simulation exercise structure

1. **Set parameters:** Choose sample size $n$ (e.g., 30) and number of replications $R$ (e.g., 5000)
2. **Create storage:** Initialize vectors to hold $R$ means and $R$ medians
3. **Run the loop:** For each replication $r = 1, \ldots, R$:
   - Generate a sample of size $n$ from `rnorm()`
   - Compute and store the sample mean
   - Compute and store the sample median
4. **Analyze results:** 
   - Calculate `var(medians) / var(means)`
   - Compare to $\pi/2 \approx 1.57$
5. **Extend:** What happens if you use `rcauchy()` instead of `rnorm()`?
:::

Take 20 minutes to prepare to report your progress to the class.
...
More generally, to estimate $E[g(X)]$ for some function $g$ and random variable $X$:
...
1.  As in the class exercise, generate $R$ samples, each a sample of size $n$, but this time use the Cauchy distribution (`rcauchy()`) rather than normal distribution (`rnorm()`). Calculate the sample variance and construct a histogram of the $R$ sample means. What do you observe?

2.  Use the bootstrap to estimate a 95% confidence interval for the median of `mtcars$mpg`. How does the width compare to a bootstrap CI for the mean?

3.  Estimate $P(Z > 4)$ using importance sampling with a shifted normal distribution centered at 4. Compare your estimate to `pnorm(4, lower.tail = FALSE)`.

4.  Suppose $(U_1, \ldots, U_n)$ are independent and identically distributed random variables following the standard uniform distribution on the interval $(0, 1)$. Now set $X_j = - \log_e(U_j)$ for $j = 1, \ldots, n$. What is the distribution of $(X_1, \ldots, X_n)$? Verify by simulation.
