...
1. **Define** entropy as a measure of uncertainty and interpret its units (bits)
2. **Calculate** entropy for simple discrete probability distributions
3. **Explain** why the joint entropy of independent variables equals the sum of their individual entropies
4. **Define** mutual information and interpret it as the reduction in uncertainty gained by knowing a related variable
5. **Interpret** KL divergence as a measure of the cost of using a suboptimal probability model
6. **Connect** entropy, mutual information, and KL divergence to machine learning applications including cross-entropy loss, decision trees, and feature selection
:::
...
In this sense, information theory exemplifies the philosophy of this book: invest in conceptual foundations not as an alternative to practical skill, but as its most durable form.
...
We suppose that the contestant devises the most informative sequence of questions possible. Consequently, the average number of required questions is a measure of the difficulty presented by the set of tickets in the box, that is, of the uncertainty of the value of a randomly drawn ticket.
...
entropy_examples |> 
  ggplot2::ggplot(mapping = ggplot2::aes(x = box, y = entropy)) + 
  ggplot2::geom_col(fill = "steelblue", width = 0.6) + 
  ggplot2::geom_text(
    mapping = ggplot2::aes(label = entropy), 
    vjust = -0.5, 
    size = 5
  ) + 
  ggplot2::labs(
...
## Exercises {#sec-info-theory-exercises}
