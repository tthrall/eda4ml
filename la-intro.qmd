...
This expression defines the following _norm_ on $n-$dimensional Euclidean space.  For any vector $v_\bullet = (v_1, \ldots, v_n) \in \mathbb{R}^n$ we define $\Vert v_\bullet \rVert_2$ as follows.
...
A norm $\Vert \cdot \rVert$ defines a corresponding distance measure $\delta_{\Vert \cdot \rVert}$
...
\begin{align} 
  \delta (m_1, m_2) &\ge 0 \\ 
  \delta (m_1, m_2) &= 0 & \text{ if and only if } m_1 = m_2 \\ 
