# Linear Regression {#sec-la-intro}
...
On the one hand, the approach to determining least-squares regression coefficients $\hat{\beta}_0, \ldots, \hat{\beta}_d$ is unchanged.  We project the response vector, now $L_\bullet$, onto the space spanned by the constant vector $1_\bullet$ along with the explanatory variables, that is onto the space spanned by $(1_\bullet, C_{\bullet, 1}, \ldots, C_{\bullet, d})$.  The fitted coefficients yield a function of the explanatory variables that forms a regression hyperplane of dimension `r n_bli_comps - 1L` that passes through a cloud of data points, $(C_{\bullet, 1}, \ldots, C_{\bullet, d}, L_\bullet)$, in a space of dimension `r n_bli_comps`.
...
The preceding section introduced example data sets along with corresponding linear regression models of the following form (the _generic linear model_). [^numeric-feature-matrix]

[^numeric-feature-matrix]: We assume in this chapter that feature matrix $X$ has only numeric columns, possibly as the result of selecting from and transforming a larger collection of numeric and non-numeric features.
...
The $n-$dimensional feature vectors $\{ x_{\bullet, k} \}_{k = 1}^d$ span a subspace, "feature space", within $n-$space, denoted $span(x_{\bullet, 1}, \ldots, x_{\bullet, d})$.  Since this is the subspace generated by the columns of matrix $X_{\bullet, \bullet}$, the subspace is also denoted as $col(X_{\bullet, \bullet})$. [^column-space]
...
#### Orthogonality
  ...
  (a) Explain why the residual vector $\epsilon_\bullet = y_\bullet - \hat{y}_\bullet$ is orthogonal to $col(X_{\bullet, \bullet})$.
...
Let: $y_\bullet^\top = (3, 1, 2)$ and $X_{\bullet, \bullet}^\top = (x_{\bullet, 1})^\top = (1, 2, 2)$.
  ...
  (a) Find the orthogonal projection $\hat{y}_\bullet$ of $y_\bullet$ onto feature space $col(X_{\bullet, \bullet})$.

  (b) Calculate the residual vector $\epsilon_\bullet = y_\bullet - \hat{y}_\bullet$.

  (c) Verify that the residual vector is orthogonal to $col(X_{\bullet, \bullet})$, that is, that $\epsilon_\bullet \perp x_{\bullet, 1}$, by computing their inner product.

  (d) Sketch vectors $(y_\bullet, x_{\bullet, 1}, \hat{y}_\bullet, \epsilon_\bullet)$ in 3D space.

#### Distance Measures

For the vectors $u_\bullet = (1,2,3)$ and $v_\bullet = (4, 1, 2)$:

  (a) Compute the Euclidean $(\mathcal{l}_2)$ distance between $u_\bullet$ and $v_\bullet$.
  (b) Compute the Manhattan $(\mathcal{l}_1)$ distance.
...
$$ {#eq-x-2D-projection}
  ...
  (a) Find the linear least-squares vector $\hat{\beta}_\bullet$ of regression coefficients by solving the set of normal equations $X_{\bullet, \bullet}^\top X_{\bullet, \bullet} \; \hat{\beta}_\bullet = X_{\bullet, \bullet}^\top \; y_\bullet$.

  (b) Calculate $\hat{y} = X_{\bullet, \bullet} \; \hat{\beta}_\bullet$.

  (c) Verify that $\hat{y}$ is in $col(X_{\bullet, \bullet})$ by expressing it as a linear combination of the columns of $X_{\bullet, \bullet}$.

  (d) Calculate $\lVert \epsilon_\bullet \rVert^2$, where $\epsilon_\bullet = y_\bullet - \hat{y}_\bullet$.
...
Using the Galton family heights data (`HistData::GaltonFamilies`):
  ...
  (a) Center the mother, father, and son height vectors by subtracting their means.
...
(a) Create a 3D plot showing the mother, father, and son centered height vectors, $(\dot{m}_\bullet, \dot{f}_\bullet, \dot{s}_\bullet)$.

(b) Add the vector of fitted values $\hat{\dot{s}}_\bullet$.

(c) Visually verify that the vector of fitted values lies in the plane spanned by the mother and father vectors.

### Advanced

#### Covariance and Inner Product

Consider the pair of $n-$dimensional vectors $(v_\bullet, w_\bullet)$ as a numeric data matrix of dimension $n \times 2$.  Recall the notation $\bar{v}$ for the arithmetic mean of $v_\bullet$, and $\dot{v}_\bullet = v_\bullet - \bar{v} \; 1_\bullet$ for the centered version of $v_\bullet$, so that $\dot{v}_i = v_i - \bar{v}$ for each element $v_i$ of $v_\bullet$.  Now the _sample covariance_ $cov(v_\bullet, w_\bullet)$ is defined as follows:
...
$$ {#eq-sample-cov-defn}
  ...
  (b) The _sample variance_ is defined as $var(v_\bullet) = cov(v_\bullet, v_\bullet)$.  Express $var(v_\bullet)$ using norm notation.
  (c) The _sample standard_ deviation is defined as $sd(v_\bullet) = \sqrt{var(v_\bullet)}$.  Express $sd(v_\bullet)$ using norm notation.
  (d) Use inner-product notation to express the _sample correlation_ $cor(v_\bullet, w_\bullet)$, defined as follows.
...
When $n-$dimensional vectors $v_\bullet, w_\bullet$ are centered $(v_\bullet \mapsto \dot{v}_\bullet)$ and normalized to unit length $(\tilde{v}_\bullet = \dot{v}_\bullet / \Vert \dot{v}_\bullet \rVert)$, show that:
  ...
  (a) The inner product $\tilde{v}_\bullet^\top \tilde{w}_\bullet$ is equal to the correlation $r$ between features $v_\bullet, w_\bullet$ times $(n - 1)$.
  (b) The angle $\theta$ between $v_\bullet, w_\bullet$ satisfies $\cos(\theta) = r$.
  (c) Apply this to the Galton height data: compute the angles between mother, father, and son height vectors.

#### Correlated Features

  (a) Generate synthetic data $(y_\bullet, x_{\bullet, 1}, x_{\bullet, 2})$ such that the two features $(x_{\bullet, 1}, x_{\bullet, 2})$ are orthogonal (correlation = 0).  Then fit a regression model.
  (b) Now generate similar data, but this time with the two features highly correlated (correlation > 0.9).  Once again, fit a regression model.
...
For any linear regression:
  ...
  (a) Show algebraically that for centered data $\lVert \dot{y}_\bullet \rVert^2 = \lVert \hat{\dot{y}}_\bullet \rVert^2 + \lVert \dot{\epsilon}_\bullet \rVert^2$.
  ...
  (d) Comment on the following characterization: (total variation) = (explained variation) + (unexplained variation).

#### Projection Matrix

The projection matrix $P$ is defined by @eq-lin-reg-projection-mat.
...
Using the Galton heights centered data:
  ...
  (a) The regression coefficients $(\hat{\beta}_m, \hat{\beta}_f)$ represent coordinates in the non-orthogonal basis defined by the mother and father vectors $(\dot{m}_\bullet, \dot{f}_\bullet)$. Express the fitted values $\hat{\dot{s}}_\bullet$ in this basis.
