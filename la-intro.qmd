...
[^n-space]: The terms $n-$dimensional space and $n-$space are colloquial versions of "an $n-$dimensional vector space $\mathcal{V}$ over the field $\mathbb{R}$ of real numbers".  Similarly, the term $n-$vector means a vector $v \in \mathcal{V}$.  Once a set of basis vectors is identified in $\mathcal{V}$, any $v \in \mathcal{V}$ can be identified by its coordinates with respect to the given basis.  This gives an isomorphism from $\mathcal{V}$ to $\mathbb{R}^n$.  Now for a numeric column of data, the successive rows of values provide an automatic representation as a vector in $\mathbb{R}^n$.  In the data set of family heights, for example, each row of data corresponds to a distinct family, and a column vector, the mother's height for example, is represented as $m_\bullet = (m_1, \ldots, m_n) \in \mathbb{R}^n$, where $m_i$ refers to the height in inches of the mother in family $i$.
...
@fig-plotly-10-22a illustrates these points.

```{r}
#| label: fig-plotly-10-22a
#| fig-cap: "plotly-10-22a"
...
@fig-plotly-10-23a illustrates these points.

```{r}
#| label: fig-plotly-10-23a
#| fig-cap: "plotly-10-23a"

# Using same centered data as above
# (mother, father, son vectors)

# Create vectors for visualization (using first 3 obs as schematic)
# mother_vec <- mother[1:3]
# father_vec <- father[1:3]
# y_vec <- son[1:3]
# y_hat_vec <- y_hat[1:3]

# use smf_smpl_ctr$yyy to define xxx_vec
mother_vec <- smf_smpl_ctr$ mother
father_vec <- smf_smpl_ctr$ father
y_vec      <- smf_smpl_ctr$ son
y_hat_vec  <- smf_smpl_ctr$ s_hat

# use smf_smpl_ctr_lm$coefficients to define beta
beta <- smf_smpl_ctr_lm$coefficients

# Create plane mesh
alpha_seq <- seq(-1.5, 1.5, length.out = 20)
beta_seq  <- seq(-1.5, 1.5, length.out = 20)

plane_x <- outer(alpha_seq, mother_vec [1], "*") + 
           outer(beta_seq,  father_vec [1], "*")

plane_y <- outer(alpha_seq, mother_vec [2], "*") + 
           outer(beta_seq,  father_vec [2], "*")

plane_z <- outer(alpha_seq, mother_vec[3], "*") + 
           outer(beta_seq,  father_vec[3], "*")

# Create the plot
g_ply_10_23a <- plot_ly() %>%
  # Add the plane (column space)
  ...
  add_trace(type = "scatter3d", mode = "lines+markers",
            x = c(0, mother_vec[1] * 1.5), 
            y = c(0, mother_vec[2] * 1.5),
            z = c(0, mother_vec[3] * 1.5),
  ...
  # Father basis vector
  add_trace(type = "scatter3d", mode = "lines+markers",
            x = c(0, father_vec[1] * 1.5), 
            y = c(0, father_vec[2] * 1.5),
            z = c(0, father_vec[3] * 1.5),
            line = list(color = "darkgreen", width = 6),
  ...
  # y_hat vector (projection)
  add_trace(type = "scatter3d", mode = "lines+markers",
            x = c(0, y_hat_vec[1]), 
            y = c(0, y_hat_vec[2]),
            z = c(0, y_hat_vec[3]),
            line = list(color = "purple", width = 8, dash = "dash"),
  ...
  add_trace(type = "scatter3d", mode = "lines",
            x = c(y_hat_vec[1], y_vec[1]), 
            y = c(y_hat_vec[2], y_vec[2]),
            z = c(y_hat_vec[3], y_vec[3]),
  ...
  layout(
    title = list(
      text = paste0("OLS Geometric Interpretation<br>",
                   "<sub>ŷ = ", round(beta[1], 2), "·mother + ", 
                   round(beta[2], 2), "·father</sub>"),
      x = 0.5
    ),
    scene = list(
      xaxis = list(title = ""),
...
@fig-s3d-10-23a illustrates these points.

```{r}
#| label: fig-s3d-10-23a
#| fig-cap: "s3d-10-23a"

# Using same centered data
# mother_vec <- mother[1:3]
# father_vec <- father[1:3]
# y_vec <- son[1:3]
# y_hat_vec <- y_hat[1:3]
...
s3d <- scatterplot3d(
  x = c(0), y = c(0), z = c(0),
  xlim = range(c(0, mother_vec[1]*2, father_vec[1]*2, y_vec[1])),
  ylim = range(c(0, mother_vec[2]*2, father_vec[2]*2, y_vec[2])),
  zlim = range(c(0, mother_vec[3]*2, father_vec[3]*2, y_vec[3])),
  type = "n",
  xlab = "", ylab = "", zlab = "",
  main = "OLS: Orthogonal Projection onto C(X)",
  sub = paste0("ŷ = ", round(beta[1], 2), "·mother + ", 
               round(beta[2], 2), "·father"),
  angle = 55,
  box = FALSE
)

# Draw plane as a grid
alpha_seq <- seq(-1, 1, length.out = 10)
beta_seq <- seq(-1, 1, length.out = 10)

for(a in alpha_seq) {
  for(b in beta_seq) {
    p1 <- a * mother_vec * 1.5 + b * father_vec * 1.5
    p2 <- a * mother_vec * 1.5 + (b + 0.2) * father_vec * 1.5
    s3d$points3d(c(p1[1], p2[1]), c(p1[2], p2[2]), c(p1[3], p2[3]),
                 type = "l", col = "lightblue", lwd = 0.5)
    
    p3 <- (a + 0.2) * mother_vec * 1.5 + b * father_vec * 1.5
    s3d$points3d(c(p1[1], p3[1]), c(p1[2], p3[2]), c(p1[3], p3[3]),
                 type = "l", col = "lightblue", lwd = 0.5)
  }
}

# Draw basis vectors
s3d$points3d(c(0, mother_vec[1]*1.5), c(0, mother_vec[2]*1.5), 
             c(0, mother_vec[3]*1.5), type = "l", col = "blue", lwd = 3)
s3d$points3d(c(0, father_vec[1]*1.5), c(0, father_vec[2]*1.5), 
             c(0, father_vec[3]*1.5), type = "l", col = "darkgreen", lwd = 3)

# Draw y vector
s3d$points3d(c(0, y_vec[1]), c(0, y_vec[2]), c(0, y_vec[3]), 
             type = "l", col = "red", lwd = 3)

# Draw y_hat vector
s3d$points3d(c(0, y_hat_vec[1]), c(0, y_hat_vec[2]), c(0, y_hat_vec[3]), 
             type = "l", col = "purple", lwd = 3, lty = 2)

# Draw residual
s3d$points3d(c(y_hat_vec[1], y_vec[1]), c(y_hat_vec[2], y_vec[2]), 
             c(y_hat_vec[3], y_vec[3]), type = "l", col = "gray40", lwd = 2)

# Add arrow endpoints
s3d$points3d(mother_vec[1]*1.5, mother_vec[2]*1.5, mother_vec[3]*1.5, 
             col = "blue", pch = 19, cex = 2)
s3d$points3d(father_vec[1]*1.5, father_vec[2]*1.5, father_vec[3]*1.5, 
             col = "darkgreen", pch = 19, cex = 2)
s3d$points3d(y_vec[1], y_vec[2], y_vec[3], 
             col = "red", pch = 19, cex = 2)
s3d$points3d(y_hat_vec[1], y_hat_vec[2], y_hat_vec[3], 
             col = "purple", pch = 19, cex = 2)

# Add labels
text(s3d$xyz.convert(mother_vec[1]*1.7, mother_vec[2]*1.7, mother_vec[3]*1.7), 
     "mother", col = "blue", cex = 1.2, font = 2)
text(s3d$xyz.convert(father_vec[1]*1.7, father_vec[2]*1.7, father_vec[3]*1.7), 
     "father", col = "darkgreen", cex = 1.2, font = 2)
text(s3d$xyz.convert(y_vec[1]*1.15, y_vec[2]*1.15, y_vec[3]*1.15), 
     "y", col = "red", cex = 1.5, font = 2)
text(s3d$xyz.convert(y_hat_vec[1]*1.15, y_hat_vec[2]*1.15, y_hat_vec[3]*1.15), 
     "ŷ", col = "purple", cex = 1.5, font = 2)

# Add legend
legend("topright", 
       legend = c("mother", "father", "y", "ŷ (projection)", "residual e"),
       col = c("blue", "darkgreen", "red", "purple", "gray40"),
       lwd = c(3, 3, 3, 3, 2),
       lty = c(1, 1, 1, 2, 1),
       cex = 0.8)
...
```{r}
#| label: mfs-coordinate-system

# Construct a non-orthogonal 3D grid in which 
# the (mother, father, son) axes of centered heights 
# are at angles whose cosines match the 
# (mother, father, son) correlations 

# Compute correlations
# cor_mf <- cor(mother, father)  # angle between mother and father
# cor_ms <- cor(mother, son)     # angle between mother and son  
# cor_fs <- cor(father, son)     # angle between father and son

# calculate covariance and correlation matrices
smf_smpl_ctr_cov <- smf_smpl_ctr |> 
  dplyr::select(mother, father, son) |> 
  cov()
smf_smpl_ctr_cor <- smf_smpl_ctr |> 
  dplyr::select(mother, father, son) |> 
  cor()
cor_mf <- smf_smpl_ctr_cor ["mother", "father"]
cor_ms <- smf_smpl_ctr_cor ["mother", "son"]
cor_fs <- smf_smpl_ctr_cor ["father", "son"]

# Place mother along x-axis
mother_axis <- c(1, 0, 0)

# Place father in xy-plane at correct angle from mother
theta_mf    <- acos(cor_mf)
father_axis <- c(cos(theta_mf), sin(theta_mf), 0)

# Place son to satisfy all three pairwise angles
# son · mother = cor_ms implies: x = cor_ms
# son · father = cor_fs implies: x*cos(theta_mf) + y*sin(theta_mf) = cor_fs
# ||son|| = 1 implies: x^2 + y^2 + z^2 = 1

x <- cor_ms
y <- (cor_fs - x * cos(theta_mf)) / sin(theta_mf)
z_squared <- 1 - x^2 - y^2

# Check if z^2 is positive (it should be if vectors are not coplanar)
if(z_squared < 0) {
  warning("Vectors are nearly coplanar")
  z <- 0
} else {
  z <- sqrt(z_squared)
}
...
# record the 3 axes as a tibble
mfs_smpl_ctr_axes <- tibble::tibble(
  m = mother_axis, 
  f = father_axis, 
  s = son_axis
)
...
```{r}
#| label: fig-s3d-10-23b
#| fig-cap: "s3d-10-23b"

# Load and prepare data
# data(GaltonFamilies)
# galton_sons <- GaltonFamilies %>%
#   filter(gender == "male") %>%
#   group_by(family) %>%
#   slice(1) %>%
#   ungroup()
...
# mother <- scale(galton_sons$mother, scale = FALSE)[,1]
# father <- scale(galton_sons$father, scale = FALSE)[,1]
# son <- scale(galton_sons$childHeight, scale = FALSE)[,1]

# Normalize to unit vectors for clean geometry
# mother <- mother / sqrt(sum(mother^2))
# father <- father / sqrt(sum(father^2))
# son <- son / sqrt(sum(son^2))

# create unit, centered vectors in new tibble
smf_smpl_unit <- smf_smpl_ctr |> 
  dplyr::select(- c(s_hat, resid)) |> 
  dplyr::mutate(across(
    .cols = c(mother, father, son), 
    .fns  = ~ .x / sqrt(sum(.x^2))
  ))
mother <- smf_smpl_unit$ mother
father <- smf_smpl_unit$ father
son    <- smf_smpl_unit$ son

# Fit regression (no intercept since centered)
model <- lm(son ~ mother + father - 1)
beta <- coef(model)
