...
Here's the linear regression problem in matrix format for a given data set of specified dimensions.
...
Let $n$ denote the number of observations (data cases), in other words the number of rows in feature matrix $X_{\bullet, \bullet}$ and in the column vectors $(y_\bullet, \epsilon_\bullet)$.  And let $d$ denote the number of columns in $X_{\bullet, \bullet}$ and the number of rows in column-vector $\beta_\bullet$.  Let us also delineate the columns of $X_{\bullet, \bullet}$, the "feature vectors", as follows.
...
The feature vectors $\{ f_{k, \bullet} \}_{k = 1}^d$ span some subspace, "feature space", within $n-$space.  Thus the mapping $\beta_\bullet \mapsto X_{\bullet, \bullet} \; \beta_\bullet$ sends a vector of coefficients $\beta_\bullet$ to the following linear combination within the feature subspace.

$$
\begin{align} 
  \beta_1 \; f_{1, \bullet} \; + \; \cdots \; + \; \beta_d \; f_{d, \bullet}
\end{align} 
$$ {#eq-generic-feature-lin-combo}

As for the dimension of the feature subspace, it can be no greater than $n$, the dimension of the space containing each feature vector, nor can it be greater than $d$, the number of feature vectors.  If the feature vectors are linearly independent (as they typically are) then this subspace is of dimension $d$.  If the feature vectors are not linearly independent (for example, if $d > n$) then the feature subspace is of some smaller dimension.
...
On the left side of the normal equations we have the matrix factor $\left ( X_{\bullet, \bullet}^\top X_{\bullet, \bullet}  \right )$.  For the centered heights data this matrix factor is a multiple of the (mother, father) covariance matrix, a $2 \times 2$ positive-definite matrix, and thus an invertible matrix. Assuming this matrix factor to be invertible, we can solve for $\hat{\beta}_\bullet$ as follows. 
...
For any value of the coefficient vector $\beta_\bullet$, the mapping $\beta_\bullet \mapsto X_{\bullet, \bullet} \beta_\bullet$ sends $\beta_\bullet$ to the linear combination $\beta_1 \; \dot{m}_\bullet \; + \; \beta_2 \; \dot{f}_\bullet$, which belongs to a 2-dimensional subspace of $n-$space [^n-space], where $n$ denotes the number of data cases, i.e., the row dimension of $X_{\bullet, \bullet}$.  This 2-dimensional subspace (let's say the "parental" subspace) is spanned by the two $n-$vectors $(\dot{m}_\bullet, \dot{f}_\bullet)$, the centered vectors of (mother, father) heights.
...
The particular coefficient vector $\hat{\beta}_\bullet$ obtained by least squares linear regression produces the linear mapping of @eq-generic-lm-y-hat:
...
For the centered heights data, this mapping sends the centered heights of sons $\dot{s}_\bullet$ to their corresponding linear prediction $\hat{\dot{s}}_\bullet = P \; \dot{s}_\bullet$.  For the generic regression problem $P$ is the following matrix.
...
That is, for any vector $v_\bullet$ in $n-$space, $P$ sends $v_\bullet$ to an $n-$vector of the form $X_{\bullet, \bullet} \; \gamma_\bullet$, which is a linear combination of $(\dot{m}_\bullet, \dot{f}_\bullet)$ and thus belongs to the parental subspace.

This means that the respective vectors of predicted response values $\hat{y_\bullet}$ and their residuals $\hat{\epsilon}_\bullet$ are mutually orthogonal.
...
Now let $p_\bullet$ be any vector in the parental space.  Then $p_\bullet$ is some linear combination of the parental vectors $(\dot{m}_\bullet, \dot{f}_\bullet)$ and therefore can be represented as $p_\bullet = X_{\bullet, \bullet} \gamma_\bullet$ for some coefficient vector $\gamma_\bullet = (\gamma_1, \gamma_2)^\top$.  It now follows the $P \; p_\bullet = p_\bullet$: 

$$
\begin{align} 
  P \; p_\bullet &= P \; (X_{\bullet, \bullet} \gamma_\bullet) \\ 
...
Consequently, the residual vector $\hat{\epsilon}_\bullet$ is orthogonal to any vector $p_\bullet = X_{\bullet, \bullet} \gamma_\bullet$ in the parental subspace: 
...
It now follows that of all vectors $p_\bullet = X_{\bullet, \bullet} \gamma_\bullet$ in the feature subspace, the predicted vector $\hat{y}_\bullet$ is closest to the given vector $y_\bullet$:

$$
\begin{align} 
  \lVert \dot{y_\bullet} - p_\bullet \rVert^2 
  &= \lVert (\dot{y_\bullet} - \hat{y}_\bullet) + (\hat{y}_\bullet - p_\bullet) \rVert^2  \\ 
  &= \lVert \hat{\epsilon}_\bullet + (\hat{y}_\bullet - p_\bullet) \rVert^2 \\ 
  &= \left ( \hat{\epsilon}_\bullet + (\hat{y}_\bullet - p_\bullet) \right )^\top 
  \left ( \hat{\epsilon}_\bullet + (\hat{y}_\bullet - p_\bullet) \right ) \\
  &= \hat{\epsilon}_\bullet^\top \hat{\epsilon}_\bullet \; + \; 0 \; + \; 0 \; + \; (\hat{y}_\bullet - p_\bullet)^\top (\hat{y}_\bullet - p_\bullet) \\ 
  &= \lVert \hat{\epsilon}_\bullet \rVert^2 \; + \; \lVert \hat{y}_\bullet - p_\bullet \rVert^2 \\
  &\ge \lVert \hat{\epsilon}_\bullet \rVert^2 \\ 
  &= \lVert \dot{y_\bullet} - \hat{y}_\bullet \rVert^2
\end{align} 
$$ {#eq-lin-reg-min-distance}

There is one more point worth noting here.  Suppose $X_{\bullet, \bullet}$ consisted of just a single column, say $\dot{m}_\bullet$.
