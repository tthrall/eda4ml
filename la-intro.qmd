...
This equation represents a plane in 3-dimensional space.  Using least-squares regression to estimate the coefficients $(\beta_0, \beta_m, \beta_f)$, we obtain a plane (@fig-g-mfs-scat3d) that gives the best linear approximation $(\hat{s})$ to the son's height for a given pair of parent heights $(m, f)$. [^least-squares-approximation]
...
#| fig-cap: "Observed (point) and predicted (plane) height of son given (mother, father) heights"
...
We'll see the problem from both angles, then focus on where the mathematics actually happens.
