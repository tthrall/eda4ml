...
In the previous section we reconstructed rows of the feature matrix $X_{\bullet, \bullet}$ using a sequence of orthogonal projections $v_{\bullet, \eta} \; v_{\bullet, \eta}^\top$.  This development gave us an eigen-decomposition of the $d \times d$ matrix $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$, the scaled feature covariance matrix.  The $d-$dimensional vector $v_{\bullet, \eta}$ turned out to be an eigenvector of this matrix, having corresponding eigenvector $\sigma_\eta^2$, indexed in descending order.  In matrix notation we have: 
...
Matrix $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$ is non-negative definite, which means that it has $d$ non-negative eigenvalues and corresponding eigenvectors.  The first $r$ eigenvalues, arranged in descending order are the positive diagonal elements of $D^2 = diag(\sigma_1^2, \ldots, \sigma_r^2)$.  If $r = d$ the matrix is strictly positive-definite, and its complete eigen-decomposition is given by @eq-cov-partial-eigen-decomp.
...
Whether or not $r = d$, we now have a complete orthonormal basis represented by matrix $V = (v_{\bullet, 1}, \ldots, v_{\bullet, d})$.  This enables us to re-express @eq-cov-partial-eigen-decomp as follows.
...
This matrix equation is meant to include the possibility that $r = d$, in which case the right side of the equation is simply $V \; D^2$.
