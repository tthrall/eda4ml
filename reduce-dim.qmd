...
Different methods optimize different objectives.  That knowledge can help one to decide which methods to use.  Here's a table that may serve as a starting point.
...
2. **Supervision**: If class labels or a response variable are available, supervised methods generally outperform unsupervised ones for prediction tasks.   However, unsupervised methods like PCA are often used for preprocessing—reducing dimension before applying a supervised classifier—especially when $d$ is large relative to $n$.
