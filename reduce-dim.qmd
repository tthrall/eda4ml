...
# Stabilize simulated data by setting 
# a seed for random number generation.

seed_eda4ml <- 37L
set.seed(seed_eda4ml)
...
**Exercise 7.3** (LDA on wine data) [^wq-data-access]

[^wq-data-access]: As noted in this book's preface, the wine quality data can be obtained from `GitHub` by installing the package `eda4mldata` accompanying the book.  In your Console run the following lines of code: (1) remotes::install_github("tthrall/eda4mldata"); (2) library(eda4mldata); (3) data(wine_quality).  Alternatively, see the Preface to download the data directly from its source.
