...
We now define PCA mathematically as a step-wise reconstruction of the $n \times d$ feature matrix $X_{\bullet, \bullet}$.  To simplify notation we assume $X_{\bullet, \bullet}$ to have already been centered: for each column of the original feature matrix the average value of the column elements has been calculated and subtracted.  Therefore the average (or equivalently the sum) of each column of $X_{\bullet, \bullet}$ equals zero.
...
Here are the distinctive details of the reconstruction process.  At step 1 we seek vector $v_{\bullet, 1}$ that minimizes the following measure of reconstruction error:

$$
\begin{align} 
  \mathcal{L} \left ( v_\bullet \; v_\bullet^\top \right ) 
  &= 
    \sum_{i = 1}^n 
    \lVert 
      x_{i, \bullet} \; - \; 
...
\end{align} 
$$ {#eq-row-projection-error-dim-1a}

In words, any unit vector $v_\bullet \in \mathbb{R}^d$ defines a projection $v_\bullet \; v_\bullet^\top$ to the 1-dimensional space of scalar multiples of $v_\bullet$.  The loss function above, $\mathcal{L} \left ( v_\bullet \; v_\bullet^\top \right )$, is the sum of squared residuals along the rows of $X_{\bullet, \bullet}$ from this projection.
...
\begin{align} 
  x_{i, \bullet} \; - \; 
      ...
      x_{i, \bullet} 
  &= 
  (I - v_\bullet \; v_\bullet^\top) \; x_{i, \bullet} 
\end{align} 
$$ {#eq-dim-1-row-projection-complement}

and that $I - v_\bullet \; v_\bullet^\top$ is also an orthogonal projection (thus symmetric and idempotent), we can re-express $\mathcal{L} \left ( v_\bullet \; v_\bullet^\top \right )$ as follows.
...
\begin{align} 
  ...
  &= 
    ...
    v_\bullet
\end{align} 
$$ {#eq-row-projection-error-dim-1b}

Now because $X_{\bullet, \bullet}$ is centered, the $d \times d$ matrix $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$ is a multiple of the sample covariance matrix among features.
...
Therefore the matrix-trace $\mathrm{Tr}$ term in @eq-row-projection-error-dim-1b is proportional to the sum of the variance of each column of the feature matrix.  @eq-row-projection-error-dim-1b also shows that 

$$
\begin{align} 
  v_{\bullet, 1} 
  &= 
  \arg \min_{ \lVert v_\bullet \rVert = 1 } \; 
  \mathcal{L} ( v_\bullet v_\bullet^\top ) \\ 
  &= 
  \arg \max_{ \lVert v_\bullet \rVert = 1 } \; 
  \left \{ 
    v_\bullet^\top \; 
      X_{\bullet, \bullet}^\top \;
      X_{\bullet, \bullet} \; 
  ...
  \right \}
\end{align} 
$$ {#eq-maximize-quad-form}

The solution to @eq-maximize-quad-form is well known.  The maximal value of $v_\bullet^\top \; X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet} \; v_\bullet$ is the largest eigenvalue, say $\sigma_1^2$, of $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$.  And the vector $v_{\bullet, 1}$ that achieves that maximal value is the eigenvector corresponding to $\sigma_1^2$.

Having defined $v_{\bullet, 1}$, we proceed inductively.  First, let $r = rank \left ( X_{\bullet, \bullet} \right )$, which is the dimension of the subspace of $\mathbb{R}^d$ spanned by the rows of $X_{\bullet, \bullet}$, as well as the dimension of the subspace of $\mathbb{R}^n$ spanned by the columns of $X_{\bullet, \bullet}$.

Suppose the we have defined an orthonormal set of vectors $v_{\bullet, 1}, \ldots, v_{\bullet, j}$, where $1 \le j \le r$.  As previously noted, this orthonormal set of vectors spans a $j-$dimensional subspace, denoted $\mathcal{R}^{(j)}$, of $\mathbb{R}^d$, and also defines an orthogonal projection $P^{(j)}$ from $\mathbb{R}^d$ to $\mathcal{R}^{(j)}$.  If $j = r$ we terminate the reconstruction.  Otherwise we define the next vector, $v_{\bullet, j + 1}$, as the unit vector that minimizes the following loss function $\mathcal{L}_{j + 1} (\cdot)$: 
...
\begin{align} 
  ...
  \left \{ 
    ...
    x_{i, \bullet} 
    \right \rVert^2 : \; 
  \lVert v_\bullet \rVert = 1 \; \land \; v_\bullet \perp v_{\bullet, \eta} \; \forall \eta
  \right \} \\ \\ 
  &\text{where} \\ \\
...
This reconstruction process terminates with an orthonormal set of $d-$dimensional vectors, $(v_{\bullet, 1}, \ldots, v_{\bullet, r})$, whose span is that of the rows of $X_{\bullet, \bullet}$.

Now let $D$ denote the $r \times r$ diagonal matrix $diag(\sigma_1, \ldots, \sigma_r)$.  Then we have 

$$
\begin{align} 
  \sum_{\eta = 1}^r \sigma_\eta^2 &= \mathrm{Tr} \left ( D^2 \right )  \\ 
  &= 
  \mathrm{Tr} 
  \left ( 
    X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet} 
...
Moreover, we have the following eigen-decomposition of the $d \times d$ matrix $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$.
