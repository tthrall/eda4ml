...
sim_1D_lst_4_1 <- sim_1D_xy_tbl()
xy_params_4_1  <- sim_1D_lst_4_1$ xy_params
xy_tbl_4_1     <- sim_1D_lst_4_1$ xy_tbl
xy_smy_4_1     <- sim_1D_lst_4_1$ xy_smy
xy_unc_4_1     <- sim_1D_lst_4_1$ xy_unc

# xy_smy_4_1
# # A tibble: 2 × 5
#   y_group   x_mean  x_sd     n propn
#   <chr>      <dbl> <dbl> <int> <dbl>
# 1 general     30.0  5.12   798 0.798
# 2 technical  101.  19.3    202 0.202
...
# change probabilities from (80, 20) to (50, 50)
xy_params_1_1 <- xy_params_4_1 |> 
  dplyr::mutate(
    prob = c(.5, .5)
  )
sim_1D_lst_1_1 <- sim_1D_xy_tbl(xy_params_1_1)
xy_tbl_1_1     <- sim_1D_lst_1_1$ xy_tbl
xy_smy_1_1     <- sim_1D_lst_1_1$ xy_smy
xy_unc_1_1     <- sim_1D_lst_1_1$ xy_unc

# xy_smy_1_1
# # A tibble: 2 × 5
#   y_group   x_mean  x_sd     n propn
#   <chr>      <dbl> <dbl> <int> <dbl>
# 1 general     29.8  4.98   510  0.51
# 2 technical  100.  20.3    490  0.49
...
# change sd's and probabilities 
xy_params_ref <- xy_params_4_1 |> 
  dplyr::mutate(
    sd   = c(10, 10), 
    prob = c(.5, .5)
  )
sim_1D_lst_ref <- sim_1D_xy_tbl(xy_params_ref)
...
#   y_group   x_mean  x_sd     n propn
#   <chr>      <dbl> <dbl> <int> <dbl>
# 1 general     29.9 10.2    476 0.476
# 2 technical  100.   9.31   524 0.524
...
# join the 3 simulated data sets
xy_tbl_df_lbl <- dplyr::bind_rows(
  xy_tbl_ref |> 
    dplyr::mutate(df_lbl = "ref"), 
  xy_tbl_1_1 |> 
    dplyr::mutate(df_lbl = "p_50_50"), 
  xy_tbl_4_1 |> 
    dplyr::mutate(df_lbl = "p_80_20")
)
...
g_xy_df_lbl <- xy_tbl_df_lbl |> 
  dplyr::mutate(df_lbl = forcats::as_factor(df_lbl)) |> 
  ggplot2::ggplot(mapping = aes(
    x = x_1, colour = y_group, fill = y_group
  )) + 
