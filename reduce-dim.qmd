...
The first principal component $(c_1)$ is a promising candidate to serve as an index of violent crime.  The coefficients of its linear combination of the three z-score variables are all positive, so that an increase in any one of the three types of crime would cause this index to rise. [^crime-index]
...
Since each feature vector $x_{\bullet, k}$ is centered, its squared norm, $\lVert x_{\bullet, k} \rVert^2$, is an $(n - 1)$ multiple of its sample variance $var(x_{\bullet, k})$.  Consequently, $c_{\bullet, 1}$ also has maximum sample variance among comparable linear combinations of feature vectors.
...
Now the coefficient vector $v_{\bullet, 1}$ is an eigenvector of matrix  $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$ whose eigenvalue is denoted $\sigma_1^2$.
...
This last equality gives a mapping $u_{\bullet, 1} \mapsto \sigma_1 \;  v_{\bullet, 1}$ from left to right singular vectors of the feature matrix.
...
[TODO: make this the last in the current sequence of subsections.]
