...
If the available data include a response variable, then the modeling goal is more focused: to determine a function of the feature matrix that closely approximates the response variable.
...
We continue to use the wine quality data to illustrate methods, beginning with linear discriminant analysis (LDA) and concluding with partial least squares (PLS).

In the wine quality principal component analysis (PCA) of the previous section, the feature matrix included wine quality and wine color.  In these new sections wine quality and wine color are removed from the feature set to play the role of responses (or labels, or target vectors).

### LDA: Linear Discriminant Analysis

In the previous section we conducted a principal component analysis (PCA) of the the wine quality data in which the first principal component separated red and white wines: scores below a certain threshold corresponded to white wines; and scores above that threshold corresponded to red wines.  Of course, this successful discernment of wine color was achieved by a linear combination of features that included wine color.

We now remove wine quality and wine color as feature columns.  We retain them in the available data for training purposes as responses to the remaining 11 features.  In this new setting, let's see how well a linear combination of the feature vectors can separate white wines from red wines.  First we develop a mathematical framework for this task.

#### Model of $K$ Categories
...
According to Bayesian decision theory (which we hereby invoke), the Bayes classification rule is to assign the $d-$dimensional observation vector $x_\bullet$ to whichever category $k$ maximizes the weighted density $\pi_k \; f_k(x_\bullet)$.
...
In this Bayesian framework the posterior probability that a new observation having feature set $\mathcal{X} = x_\bullet$ belongs to class $Y = \mathcal{C}_\eta$ is: 
...
wq_2D_lda_color_mdl <-
  ...
  parsnip::fit_xy(
    x = wq_z |> 
      dplyr::select(res_sugar, density), 
    y = wq_z$ color |> 
      forcats::as_factor()
  )
...
wq_2D_LD1_coeff <- 
  wq_2D_lda_color_mdl$ fit$ scaling

# wq_2D_LD1_coeff |> str()
#  num [1:2, 1] 1.67 -1.72
#  - attr(*, "dimnames")=List of 2
#   ..$ : chr [1:2] "res_sugar" "density"
#   ..$ : chr "LD1"
...
```{r}
#| label: wq-2D-lda-color-predict

# tibble of 1 column <fct>: .pred_class (red or white)
wq_2D_lda_color_class <- wq_2D_lda_color_mdl |> 
...
wq_2D_lda_color_prob <- wq_2D_lda_color_mdl |> 
  parsnip::predict.model_fit(
    new_data = wq_z |> 
      dplyr::select(res_sugar, density), 
    type = "prob")

wq_2D_lda_color_predict <- dplyr::bind_cols(
  wq_z  |> 
    dplyr::select(
      res_sugar, 
      density, 
      color), 
  wq_2D_lda_color_class, 
  wq_2D_lda_color_prob) |> 
  dplyr::rename(
    class   = .pred_class, 
    p.red   = .pred_red, 
    p.white = .pred_white
...
```{r}
#| label: wq-2D-lda-color-long

# long version for violin plot, etc.
wq_2D_lda_color_long <- wq_2D_lda_color_predict |> 
  tidyr::pivot_longer(
...
[^homogeneous-coefficients]: The table gives the homogeneous coordinates of the LD1 line in the form of $c_0 \; + \; c_1 \; x_{\bullet, 1} \; + \; c_2 \; x_{\bullet, 2} = 0$.  Unfortunately, the constant coefficient $c_0$ was calculated but not listed in the output.
...
wq_2D_lda_x_means    <- wq_2D_lda_lst$ x_means
wq_2D_lda_x_cov      <- wq_2D_lda_lst$ x_cov
wq_2D_lda_grp_stats  <- wq_2D_lda_lst$ grp_stats
wq_2D_lda_coeff_tbl  <- wq_2D_lda_lst$ coeff_tbl
wq_2D_lda_coeff_diff <- wq_2D_lda_lst$ coeff_diff

# wq_2D_lda_coeff_diff
# # A tibble: 1 × 5
...
@fig-wq-2D-lda-xy shows a scatter diagram of density and res-sugar, along with the estimated LDA line separating red wines from white wines.  Points corresonding to red wines are shown in red.

```{r}
#| label: fig-wq-2D-lda-xy
#| fig-cap: "Wine color: linear discriminant function LD1"

g_wq_2D_lda_xy <- wq_2D_lda_color_predict |>
  ggplot2::ggplot(mapping = aes(
  ...
  ggplot2::geom_abline(
    ...
    intercept = 
      - wq_2D_lda_coeff_diff [[1, "const"]] / 
        wq_2D_lda_coeff_diff [[1, "res_sugar"]], 
    slope     = 
      - wq_2D_lda_coeff_diff [[1, "density"]] / 
        wq_2D_lda_coeff_diff [[1, "res_sugar"]] ) + 
  ggplot2::labs(
...
Notable here are the outlying values.  These certainly merit further inspection, but we set aside that investigation for now.

#### Wine Quality: $d = 2$, $K = 4$

#### Wine Quality: $d = 11$, $K = 2$

```{r}
#| label: wq-lda-color-mdl
...
```{r}
#| label: wq-lda-color-predict

# tibble of 1 column <fct>: .pred_class (red or white)
wq_lda_color_class <- wq_lda_color_mdl |> 
  parsnip::predict.model_fit(
    new_data = wq_z |> 
      dplyr::select(- c(quality, color)), 
    type = "class")
...
# tibble of 2 columns <dbl>: (.pred_red, .pred_white)
wq_lda_color_prob <- wq_lda_color_mdl |> 
  parsnip::predict.model_fit(
    new_data = wq_z |> 
      dplyr::select(- c(quality, color)), 
    type = "prob")

wq_lda_color_predict <- dplyr::bind_cols(
  wq_z, wq_lda_color_class, wq_lda_color_prob)

```

To facilitate the interpretation of the LDA linear discriminant function (`LD1`), we re-scale the values of each feature to z-values, obtained by subtracting the mean and dividing by the standard deviation.  The resulting coefficients fit by `R` function `MASS::lda()` are shown in @tbl-lda-coefficients.

```{r}
#| label: tbl-lda-coefficients
#| tbl-cap: "Wine Color LDA: model coefficients"

wq_lda_color_mdl$ fit$ scaling |> 
  tibble::as_tibble(rownames = "feature") |> 
  dplyr::mutate(
...
The coefficients having a magnitude of nearly 1 or greater correspond to the following color-distinguishing features: residual sugar, total sulfur dioxide, density, and alcohol.

Now let's see how well this linear discriminant function, LD1, distinguished the actual red and white wines.  @tbl-lda-xtabs-color is a cross-tabulation of actual versus predicted color.

```{r}
#| label: tbl-lda-xtabs-color
#| tbl-cap: "Wine Color: actual versus LDA prediction"

wq_lda_color_xtabs_long <- wq_lda_color_predict |> 
  dplyr::summarise(
    .by = c(color, .pred_class), 
    n = n())

wq_lda_color_xtabs_wide <- wq_lda_color_xtabs_long |> 
  tidyr::pivot_wider(
    names_from = "color", 
    values_from = "n")

wq_lda_color_xtabs_wide |> 
  knitr::kable(
...
The mis-classification rates are remarkably low: about 1% for red wines, and 0.4% for white wines.
...
```{r}
#| label: fig-wq-lda-color-probs
#| fig-cap: "Color probabilies per actual and predicted color"

g_wq_lda_color_probs <- wq_lda_color_probs |> 
...
We now move from the classification of wine color to the prediction of wine quality.
