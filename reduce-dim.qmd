...
In the wine quality PCA of the previous section, the first principal component separated red and white wines: scores below a certain threshold corresponded to white wines; and scores above that threshold corresponded to red wines.  Of course, this successful discernment of wine color was achieved by a linear combination of features that included wine color.

We now remove wine quality and wine color as feature columns.  We retain them in the available data for training purposes, but set them aside as responses to the remaining 11 features.  In this new framework, let's see how well a linear combination of the feature vectors can separate white wines from red wines.  First we develop a mathematical framework for this task.

#### LDA Model

The basic LDA model assumes that each observation, that is, each row $x_{i, \bullet}$ of the feature matrix $X_{\bullet, \bullet}$ belongs to one of $K$ categories, $(\mathcal{C}_1, \ldots, \mathcal{C}_K)$.  A response or labeling variable $Y$ (available in the training data) identifies the category to which each row belongs. [^K-2-wine-color] [^label-format]  

[^K-2-wine-color]:  For the wine data, $K = 2$, since the categories are red wine and white wine.

[^label-format]: $Y$ can be constructed either as a simple label, say $Y \in \{ 1, \ldots, K \}$ or as an indicator vector $Y \in \{ e_{\bullet, 1}, \ldots, e_{\bullet, K} \}$, where $e_{\bullet, k} \in \mathbb{R}^K$ is the $k^{th}$ Euclidean basis vector and signals membership in category $k$. 
...
According to Bayesian decision theory (which we hereby invoke), the Bayes classification rule is to assign the $d-$dimensional observation vector $x_\bullet$ to whichever category $k$ maximizes the weighted density $\pi_k \; f_k(x_\bullet)$.  This amounts to the maximization of: 
...
The LDA model implements this rule by replacing each unknown parameter value with the corresponding value estimated from the data:
...
wq_lda_color_predict <- dplyr::bind_cols(
  wine_quality, wq_lda_color_class, wq_lda_color_prob)

```

@tbl-lda-coefficients shows the coefficients fit by `R` function `MASS::lda()`.
