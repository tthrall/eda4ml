...
There are two distinct approaches to calculating principal components: 
  ...
  2. _SVD_ (singular value decomposition): Based on the matrix factorization $X_{\bullet, \bullet} = U_{\bullet, \bullet} \; \Sigma_{\bullet, \bullet} \; V_{\bullet, \bullet}^\top$, calculate singular value $\sigma_\eta$ and corresponding left singular vector $u_{\bullet, \eta}$ in order to construct principal component $c_{\bullet, \eta} = \sigma_\eta \; u_{\bullet, \eta}$.

If the number of features $d$ is large, then EVD is cumbersome. Actually either approach requires careful planning for a large feature matrix, but SVD is typically used.  It facilitates computation of just some, not all, of the principal components.  For example, SVD is the approach used by `R` function `stats::prcomp()`.  See @Martin_2012_SVD for an overview of applications and extensions of SVD algorithms.
