...
Our construction of principal components also relies on the notion of an orthogonal projection $P$ from $\mathbb{R}^d$ to some subspace of $\mathbb{R}^d$.  Recall that $P$ is an orthogonal projection if and only if: it is idempotent $(P^2 = P)$; and it is symmetric  $(P^\top = P)$.  Also, if $P$ is an orthogonal projection then so is its complement $\mathcal{I} - P$.

The simplest example is the $d \times d$ matrix $v_\bullet \; v_\bullet^\top$, where $v_\bullet \in \mathbb{R}^d$ is a unit vector (a vector of unit norm).  This sends $x_\bullet \in \mathbb{R}^d$ to a scalar multiple of $v_\bullet$, namely $\left < x_\bullet, v_\bullet \right > v_\bullet$.
...
Thus $\mathcal{L} (P)$ is a sum over row index $i$ of squared errors in the approximation of $x_{i, \bullet}$ by $P \; x_{i, \bullet}$.

Now let $\mathcal{P}$ be a family of projections subject to some constraints.  From @eq-row-projection-squared-error we see that: 
...
Here are the distinctive details of the reconstruction process.  At step 1 we define vector $v_{\bullet, 1}$ as the unit vector that minimizes $\mathcal{L} (v_\bullet \; v_\bullet^\top)$ among all unit vectors $v_\bullet \in \mathbb{R}^d$: 
...
Note that the $d \times d$ matrix $X_{\bullet, \bullet}^\top \; X_{\bullet, \bullet}$ is non-negative definite, so that its complete set of eigenvalues consist of the positive diagonal elements of $D^2 = diag(\sigma_1^2, \ldots, \sigma_r^2)$, along with $d - r$ zeros, if $r < d$.  The corresponding eigenvectors are $(v_{\bullet, 1}, \ldots, v_{\bullet, r})$ along with the possibility (if $r < d$) of $d - r$ unit vectors, say $(v_{\bullet, r + 1}, \ldots, v_{\bullet, d})$ that complement the first set so that $V = (v_{\bullet, 1}, \ldots, v_{\bullet, d})$ is a complete orthonormal basis for $\mathbb{R}^d$.  Restricting attention to just the non-zero eigenvalues, we have:
