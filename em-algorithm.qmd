# EM: the Expectation-Maximization Algorithm {#sec-em-algorithm}
...
[^faithful-chambers]: Old Faithful eruptions are conjectured to occur in 2 distinct temporal patterns due to the presence of an upper and lower chamber beneath the vertical column (tube) that forms the geyser.  See @Vandemeulebrouck_Roux_Cros_2013.
...
Fitting a single multivariate normal distribution is straightforward[^art-mv-normal-est], but less so for the above mixture of bivariate normal distributions. The Expectation-Maximization (EM) algorithm provides an approach for doing so, as follows.

[^art-mv-normal-est]:  See @Anderson_Olkin_2002.
...
g_faithful_2 <- faithful_2 |> 
  ...
  ggplot2::geom_point(
    mapping = ggplot2::aes(colour = g, shape = g), 
    show.legend = TRUE
  ) + 
...
We illustrate the iterations of the EM algorithm using function `mvnormalmixEM()` in R package `mixtools`.[^art-mixtools] We provide the function with our initial estimates of normal parameters.

[^art-mixtools]:  See @mixtools2009.
