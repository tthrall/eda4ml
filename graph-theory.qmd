...
1. **Represent** networks as graphs with nodes, edges, and matrix representations
2. **Compute** centrality measures (degree, betweenness, closeness, PageRank) and interpret their meanings
3. **Detect** communities using modularity-based algorithms and evaluate partition quality
4. **Analyze** bipartite graphs and construct projections for recommendation systems
5. **Navigate** directed acyclic graphs (DAGs) for dependency and prerequisite reasoning
6. **Build** feature matrices from graph structure for machine learning tasks
:::
...
@tbl-lg-schema summarizes the LearningGraph structure. [^skill-levels]

[^skill-levels]: To simplify the exposition we depart from common practice in organizational psychology, where proficiency levels pertain to _competencies_ (groups of skills), rather than the skills themselves.
